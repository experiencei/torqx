...
import { 
  Menu, Upload, Search, MoreHorizontal, Play, Image as ImageIcon, 
  FileVideo, Maximize, Tag, Link2, Plus, Eye, Edit3, Trash2
} from "lucide-react";
...
export function GalleryPage({ sidebarCollapsed, setSidebarCollapsed }: GalleryPageProps) {
    ...
    async function fetchCreatives() {
      try {
        ...
        const res = await databases.listDocuments(
          ...
          [Query.equal("userId", user.$id)] // only current user’s creatives
        );
        ...
        const mapped = res.documents.map((doc: any) => ({
          ...
          type: doc.type,
          size: doc.size,
          url: storage.getFileView(process.env.NEXT_PUBLIC_BUCKET_ID!, doc.fileId),
          tags: doc.tags || [],
  ...
  if (loading) {
    return <div className="p-8">Loading creatives...</div>;
  }
    ...
    <>
      ...
      <header className="px-4 md:px-8 py-6 border-b border-gray-200 bg-white">
        <div className="flex items-center justify-between">
          ...
          <h1 className="text-xl font-semibold text-black">Creative Gallery</h1>
      ...
      {/* Main Content */}
      <div className="p-8 bg-gray-50 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredCreatives.map((creative) => (
          <Card key={creative.$id} onClick={() => handleCreativeClick(creative)} className="cursor-pointer">
            <img src={creative.url} alt={creative.name} className="rounded-t-lg object-cover w-full h-48" />
            <div className="p-4">
              <h3 className="font-medium text-black truncate">{creative.name}</h3>
        ...
        <DialogContent>
          {selectedCreative && (
            <>
              ...
              <img src={selectedCreative.url} alt={selectedCreative.name} className="rounded-lg max-h-[70vh]" />
            </>
          )}
        </DialogContent>
      </Dialog>
